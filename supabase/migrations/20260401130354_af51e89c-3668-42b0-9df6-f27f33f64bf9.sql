...
-- Fix both overloads of create_prompt to insert content into prompt_versions

CREATE OR REPLACE FUNCTION public.create_prompt(p_title text, p_description text, p_price numeric, p_category_id integer, p_ai_model_target text, p_content text, p_suggested_category text DEFAULT NULL::text)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Store the actual prompt content in prompt_versions
...
-- Also fix the overload without suggested_category
